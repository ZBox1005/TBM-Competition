...
# ================== DATA PREPARATION ====================
# 读取表格数据
# data_df = pd.read_csv('input_output_normalized.csv')
# train_df, val_df = train_test_split(data_df, test_size=0.1, random_state=42)
# val_df = pd.read_csv('test_data_6_normalized.csv')

# train_df.dropna(inplace=True)
# val_df.dropna(inplace=True)
# train_deleted_rows = len(train_df) - len(train_df.dropna())
# val_deleted_rows = len(val_df) - len(val_df.dropna())
# print(f"Deleted {train_deleted_rows} train rows and {val_deleted_rows} val rows with NaN values.")
...
input_size = features.shape[1]  # 输入维度
encoding_size = 10  # 编码维度
...
# output_size = 4  # 四分类任务
output_size = 1  # 二分类任务
mlp = MLP(encoding_size, hidden_size, output_size)

# 定义损失函数和优化器
# mlp_criterion = nn.CrossEntropyLoss() # 四分类
mlp_criterion = nn.BCEWithLogitsLoss() # 二分类
# 按样本数量设置每个类别的代价权重
# class_weights = torch.tensor([1/(class_counts[0]), 1/class_counts[1], 1/(class_counts[2]), 1/class_counts[3]], dtype=torch.float)
# class_weights = torch.tensor([0.1, 0.5, 0.5, 1.0])
...
for epoch in range(num_epochs_ae):
    ...
    for i, (inputs, labels) in enumerate(train_loader):
        ...
        loss = nn.MSELoss()(autoencoder.decoder(encoded_data), inputs)
    ...
    if (epoch + 1) % 100 == 0:
        print(f'Autoencoder Epoch [{epoch + 1}/{num_epochs_ae}], Loss: {loss.item():.4f}')
...
best_mlp = MLP(encoding_size, hidden_size, output_size)  # 请替换为你的MLP模型定义
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.savefig('Loss/loss_32ae_64mlp_200e_128bs_-4lr_wcls.png')
plt.close()
...
plt.savefig('Acc/acc_32ae_64mlp_200e_128bs_-4lr_wcls.png')
plt.close()